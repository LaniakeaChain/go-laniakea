...
func stackTrieFromPool(db lkeadb.KeyValueWriter, owner common.Hash) *StackTrie {
...
type StackTrie struct {
	...
	db       lkeadb.KeyValueWriter // Pointer to the commit db, can be nil
}

// NewStackTrie allocates and initializes an empty trie.
func NewStackTrie(db lkeadb.KeyValueWriter) *StackTrie {
	return &StackTrie{
		nodeType: emptyNode,
		db:       db,
	}
...
func NewStackTrieWithOwner(db lkeadb.KeyValueWriter, owner common.Hash) *StackTrie {
	return &StackTrie{
		owner:    owner,
		nodeType: emptyNode,
		db:       db,
...
func NewFromBinary(data []byte, db lkeadb.KeyValueWriter) (*StackTrie, error) {
	var st StackTrie
...
func (st *StackTrie) setDb(db lkeadb.KeyValueWriter) {
	st.db = db
	for _, child := range st.children {
		if child != nil {
			child.setDb(db)
...
func newLeaf(owner common.Hash, key, val []byte, db lkeadb.KeyValueWriter) *StackTrie {
...
func newExt(owner common.Hash, key []byte, child *StackTrie, db lkeadb.KeyValueWriter) *StackTrie {
	st := stackTrieFromPool(db, owner)
	st.nodeType = extNode
	st.key = append(st.key, key...)
	st.children[0] = child
