...
import (
	...
	"github.com/LaniakeaChain/go-laniakea/common/mclock"
	"github.com/LaniakeaChain/go-laniakea/common/prque"
	...
	"github.com/LaniakeaChain/go-laniakea/core/state/snapshot"
	"github.com/LaniakeaChain/go-laniakea/core/types"
	"github.com/LaniakeaChain/lkeavm/vm"
	"github.com/LaniakeaChain/go-laniakea/lkeadb"
	"github.com/LaniakeaChain/go-laniakea/event"
	"github.com/LaniakeaChain/go-laniakea/internal/syncx"
	...
	"github.com/LaniakeaChain/go-laniakea/trie"
...
type BlockChain struct {
	...
	db     lkeadb.Database // Low level persistent database to store final content in
...
// and Processor.
func NewBlockChain(db lkeadb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, root common.Hash, repair bool) (uint64, error) {
	...
	updateFn := func(db lkeadb.KeyValueWriter, header *types.Header) (uint64, bool) {
	...
	delFn := func(db lkeadb.KeyValueWriter, hash common.Hash, num uint64) {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			if batch.ValueSize() > lkeadb.IdealBatchSize || i == len(blockChain)-1 {
	...
	writeLive := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// tx indexes)
			if batch.ValueSize() >= lkeadb.IdealBatchSize {
				if err := batch.Write(); err != nil {
					return 0, err
				}
				size += int64(batch.ValueSize())
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - lkeadb.IdealBatchSize)
			}
