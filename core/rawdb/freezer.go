...
//   reserving it for go-laniakea. This would also reduce the memory requirements
...
func (f *Freezer) ReadAncients(fn func(lkeadb.AncientReaderOp) error) (err error) {
	f.writeLock.RLock()
	defer f.writeLock.RUnlock()
...
func (f *Freezer) ModifyAncients(fn func(lkeadb.AncientWriteOp) error) (writeSize int64, err error) {
	if f.readonly {
