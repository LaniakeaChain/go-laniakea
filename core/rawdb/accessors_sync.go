...
func ReadSkeletonSyncStatus(db lkeadb.KeyValueReader) []byte {
	data, _ := db.Get(skeletonSyncStatusKey)
	return data
}

// WriteSkeletonSyncStatus stores the serialized sync status to save at shutdown.
func WriteSkeletonSyncStatus(db lkeadb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSkeletonSyncStatus(db lkeadb.KeyValueWriter) {
...
func ReadSkeletonHeader(db lkeadb.KeyValueReader, number uint64) *types.Header {
...
func WriteSkeletonHeader(db lkeadb.KeyValueWriter, header *types.Header) {
...
func DeleteSkeletonHeader(db lkeadb.KeyValueWriter, number uint64) {
