...
func ReadDatabaseVersion(db lkeadb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db lkeadb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db lkeadb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db lkeadb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
// given genesis hash.
func ReadGenesisStateSpec(db lkeadb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(genesisStateSpecKey(hash))
	return data
}

// WriteGenesisStateSpec writes the genesis state specification into the disk.
func WriteGenesisStateSpec(db lkeadb.KeyValueWriter, hash common.Hash, data []byte) {
...
func PushUncleanShutdownMarker(db lkeadb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db lkeadb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
...
func UpdateUncleanShutdownMarker(db lkeadb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
...
func ReadTransitionStatus(db lkeadb.KeyValueReader) []byte {
	data, _ := db.Get(transitionStatusKey)
	return data
}

// WriteTransitionStatus stores the eth2 transition status to the database
func WriteTransitionStatus(db lkeadb.KeyValueWriter, data []byte) {
