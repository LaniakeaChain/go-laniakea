...
func CheckDanglingStorage(chaindb lkeadb.KeyValueStore) error {
...
func checkDanglingDiskStorage(chaindb lkeadb.KeyValueStore) error {
	var (
		lastReport = time.Now()
		start      = time.Now()
		lastKey    []byte
...
// snapshot difflayers.
func checkDanglingMemStorage(db lkeadb.KeyValueStore) error {
	start := time.Now()
...
func CheckJournalAccount(db lkeadb.KeyValueStore, hash common.Hash) error {
