// Copyright 2021 The go-laniakea Authors
...
func showLeveldbStats(db lkeadb.KeyValueStater) {
...
type snapshotIterator struct {
	init    bool
	account lkeadb.Iterator
	storage lkeadb.Iterator
}
...
var chainExporters = map[string]func(db lkeadb.Database) utils.ChainDataIterator{
	"preimage": func(db lkeadb.Database) utils.ChainDataIterator {
		iter := db.NewIterator(rawdb.PreimagePrefix, nil)
		return &preimageIterator{iter: iter}
	},
	"snapshot": func(db lkeadb.Database) utils.ChainDataIterator {
...
func dbHasLegacyReceipts(db lkeadb.Database, firstIdx uint64) (bool, uint64, error) {
	// Check first block for legacy receipt format
	numAncients, err := db.Ancients()
	if err != nil {
		return false, 0, err
